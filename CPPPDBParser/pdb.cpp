...
extern void (_cdecl *PrintfCallout)(const char*, ...);
...
//
PCSTR PdbpGetImagePdbName (PVOID Image)
{
	PRSDSI rsds = PdbpGetImageRsds (Image);
	return rsds ? (PCSTR)rsds->szPdb : NULL;
}
...
//
PPDB PdbOpenValidate (const char *szPdbFileName, GUID* guidSig OPTIONAL, DWORD age OPTIONAL)
{
	PPDB pdb;
	...
	if (pdb != NULL)
	{
		...
		if (pdb->msf != NULL)
		{
			...
			if (pdb->Symd != NULL)
			{
				...
				GetFullPathNameA(szPdbFileName, MAX_PATH-1, pdb->szPdbFileName, 0);
...
//
PPDB PdbOpen (const char *szPdbFileName)
{
...
//
static PPDB PdbpFindInDirectory (PRSDSI rsds, PCSTR szPdbName, PCSTR szSymbolSearchPath)
{
	...
	SetCurrentDirectoryA(szSymbolSearchPath);
	...
	if (hSearch)
	{
		do 
		{
			...
			else if (!_stricmp (PathFindExtensionA(wfd.cFileName), ".pdb"))
			{
				// Try this file.
...
//
PPDB PdbpTryOpenFromStorage (PVOID Image, PCSTR szSymbolSearchPath)
{
	char PreviousDirectory[MAX_PATH];
...
//
PPDB PdbFindMatchingPdbForImage (PVOID Image, PCSTR szSymbolSearchPath)
{
	WIN32_FIND_DATAA wfd = { 0 };
	PCSTR szPdbName = 0;
	char PreviousDirectory[MAX_PATH] = { 0 };
	PRSDSI rsds = 0;
	PPDB pdb = NULL;
	HANDLE hSearch = 0;
	rsds = PdbpGetImageRsds (Image);
	...
	if (PathIsRelativeA(szPdbName) == FALSE)
	{
	...
	char *szRelativePDBName = PathFindFileNameA(szPdbName);
	...
	// Enumerate all PDBs...
	hSearch = FindFirstFileA ("*.pdb", &wfd);
	if (hSearch)
	{
		do 
		{
			PCSTR ext = PathFindExtensionA (wfd.cFileName);
			if (!_stricmp (ext, ".pdb"))
			{
...
//
PPDB PdbFindMatchingPdbForImageFile (PCSTR szImageFileName, PCSTR szSymbolSearchPath)
{
	PVOID Image;
	PPDB Pdb = NULL;
...
{
	PCSTR szPdbPath = PdbpGetImagePdbName (Image);
