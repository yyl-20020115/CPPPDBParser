...
VOID SYMpDumpSymbol (PSYMD Symd, PSYM Sym)
{
	...
	case S_GDATA32:
		{
		const char *type = TPILookupTypeName (Symd->TpiHdr, Sym->Data32.typind);
			const char *decl = TPIGetSymbolDeclaration (Symd->TpiHdr, type, (char*)Sym->Data32.name);
...
VOID SYMFreeSymbol (PSYMBOL Symbol)
{
	if (Symbol->FullSymbolDeclaration)
		hfree ((void*)Symbol->FullSymbolDeclaration);

	if (Symbol->SymbolName)
		hfree ((void*)Symbol->SymbolName);
