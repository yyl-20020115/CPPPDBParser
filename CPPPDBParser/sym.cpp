...
void _cdecl PrintfNull (const char*, ...)
{
	// do nothing.
}

void (_cdecl *PrintfCallout)(const char*, ...) = &PrintfNull;
...
std::vector<PPDB_SYMBOLS> SymbolsListHead;

static char szSymbolPath[MAX_PATH];

VOID SymSetSymbolPath (PCSTR SymbolPath)
{
...
--*/
{
	...
	for(PPDB_SYMBOLS syms : SymbolsListHead)
	{
...
--*/
{
	...
	if (syms != NULL)
	{
		...
		if (pdb != NULL)
		{
			...
			SymbolsListHead.push_back(syms);
...
PPDB_SYMBOLS SymLoadSymbolsForImageFile (PCSTR szFileName)
/*++
...
--*/
{
	...
	auto it = std::find(SymbolsListHead.begin(),SymbolsListHead.end(), syms);
	if(it!=SymbolsListHead.end())
		SymbolsListHead.erase(it);
...
--*/
{
	hfree ((void*)Url->szProtocol);
}

static BOOL CrackUrl (PCSTR szServerUrl, PCRACKED_URL Url)
/*++
	Crack URL and split into the parts.
	Cracked url must be freed by FreeUrl()
--*/
{
	PCSTR szUrl = xstrdup (szServerUrl);

	PSTR pProto = (char*)strstr (szUrl, "://");
	if (pProto)
	{
		...
		*pProto = 0;
		...
		if (pFirstSlash)
		{
			...
			memmove ((void*) & pProto[2], &pProto[3], Len);
			pFirstSlash[-1] = 0;
		...
		PSTR pPort =(char*) strchr (Url->szHostName, ':');
		if (pPort)
		{
			*(pPort++) = 0;
...
static BOOL RequestSucceeded (PCSTR response)
{
	PCSTR pSpace = strchr (response, ' ');
	if (pSpace != NULL)
	{
		pSpace++;
...
static BOOL MakePath (PCSTR szSymSignature)
{
	PCSTR pSlash, pPrev = szSymSignature;
...
static BOOL SymSocketSaveTo (PCSTR bufferTail, int tailLength, int sock, PCSTR szFilePath, ULONG Length)
{
	int bytesRead = tailLength;
	DWORD wr;
	BOOL bResult = FALSE;
	HANDLE hFile = CreateFileA (szFilePath, GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_ALWAYS, 0, 0);
...
static BOOL SympHttpHandleConnection (PCSTR szSymSignature, int sock, PCRACKED_URL Url, PCSTR szLocalStorage)
{
				...
				if (RequestSucceeded (buffer))
				{
					...
					if (pHeaderEnd != NULL)
					{
						...
						if (szContentLength != NULL)
						{
							...
							GetCurrentDirectoryA (sizeof(PreviousDirectory)-1, PreviousDirectory);
							SetCurrentDirectoryA (szLocalStorage);
...
BOOL SymDownloadFromServerPdb (PCSTR szPdbSignature, PCSTR szServerUrl, PCSTR szLocalStorage)
{
	BOOL bResult = FALSE;
...
BOOL SymDecompressPdb (PCSTR szCompressedName, PCSTR szPdbName, PCSTR szLocalStorage)
{
	...
	if (CreateProcessA(0, commandLine, 0, 0, FALSE, CREATE_NO_WINDOW, 0, szLocalStorage, &si, &pi))
	{
...
BOOL SymDownloadFromServerForImage (PVOID Image, PCSTR szServerUrl, PCSTR szLocalStorage)
{
	WSADATA WsaData;
	BOOL bResult = FALSE;
