...
extern ULONG rgphrBuckets[iphrMax];
...
void DumpStreams (char *name, PPDB_SYMBOLS syms)
{
	for (ULONG i=0; i<syms->pdb->msf->Root->dwStreamCount; i++)
	{
		...
		sprintf (szFile, "P:\\pdbparser\\%s.%03d", name, i);

		HANDLE hFile;
		SIZE_T size;
		PVOID pStream = MsfReferenceStream (syms->pdb->msf, i, &size);
		DWORD fs = (DWORD)size;
		hFile = CreateFileA (szFile, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, 0, 0);
		WriteFile (hFile, pStream, size, &fs, 0);
		CloseHandle (hFile);
...
void FromNtPath()
{
	...
	GetEnvironmentVariableA ("_NT_SYMBOL_PATH", NtSymbolPath, sizeof(NtSymbolPath));
...

	{
		...
		{
			PCSTR pPath = &NtSymbolPath[4];
			PCSTR pAsterisk = strchr (pPath, '*');
			if (pAsterisk)
			{
...
void walkstack()
{
	...
	StackFrame.AddrStack.Offset = (DWORD)(ULONG_PTR)Context.Rsp;
	StackFrame.AddrPC.Mode = AddrModeFlat;
	StackFrame.AddrPC.Offset = (DWORD64)(ULONG_PTR)Context.Rip;
	StackFrame.AddrFrame.Mode = AddrModeFlat;
	StackFrame.AddrFrame.Offset = (DWORD64)(ULONG_PTR)Context.Rbp;
#else
	StackFrame.AddrStack.Mode = AddrModeFlat;
	StackFrame.AddrStack.Offset = (DWORD)(ULONG_PTR)Context.Esp;
	StackFrame.AddrPC.Mode = AddrModeFlat;
...
// 	SYMDumpSymbols (syms->pdb->Symd, S_ALL);
	...
	while (StackWalk64 (IMAGE_FILE_MACHINE_I386, GetCurrentProcess(), GetCurrentProcess(), &StackFrame, 0, 0, 0, 0, 0))
	{
		...
		PCSTR szBaseModuleName = NULL;

		if (Base)
		{
			GetModuleFileNameA ((HMODULE)Base, moduleName, sizeof(moduleName)-1);
			szBaseModuleName = PathFindFileNameA (moduleName);
...
void PrintEntryPointNameForImage (const char *ImageName)
{
	char windir[MAX_PATH];
	GetWindowsDirectoryA (windir, sizeof(windir)-1);
...
void test_selfdll()
{
	...
	Image = GetModuleHandleA("symeng.dll");
	syms = SymLoadSymbolsForImage (Image);
...
void test_another_mod_ep (const char *mod)
{
	PPDB_SYMBOLS syms;
	PVOID Image;
	SIZE_T FileSize;
...
int main (int argc, char **argv)
{
	...
	syms = SymLoadSymbolsForImage (GetModuleHandleA("kernel32.dll"));
	if (syms == NULL)
